"""
Práctica 3: Algoritmos de regresion no lineal

Departamento de Ingeniería Eléctrica y Electrónica, Ingeniería Biomédica
Tecnológico Nacional de México [TecNM - Tijuana]
Blvd. Alberto Limón Padilla s/n, C.P. 22454, Tijuana, B.C., México

Nombre del alumno: Alain Yahir Chaparro Zamora
...
def plotdata(t,x1,x2,xo):
    ...
    plt.plot(t,x1,linestyle = 'none', marker = 'x', 
             color = [0.5, 0.0, 0.5], label = '$x_1(t)$')
    plt.plot(t,x2,linestyle = 'none', marker = 'x', 
             color = [0.0, 0.0, 0.0], label = '$x_2(t)$')
    plt.plot(t,xo,'-o',linewidth = 0.5, markersize = 3, 
             color = [0.51, 0.30, 0.90], label = '$x_o(t)$')
    plt.xlabel('$t$ $[months]$')
...
def plotresults(t, xo, xa, S):
    ...
    plt.rcParams.update({'font.size': 11})
    fig = plt.figure(figsize=(8, 4))
    plt.plot(t, xo, linestyle='none', marker='x', color=[0.11, 0.30, 0.42], label='$x_o(t)$')
    plt.plot(t, xa, linewidth=0.5, markersize=3, color=[0.11, 0.30, 0.42], label='$x_a(t)$ (Model)')
    plt.xlabel('$t$ $[months]$')
    plt.ylabel('$x_i(t)$ $[cells]$')
    plt.xlim(0, 72)
    xticks = np.arange(0, 73, 8)
